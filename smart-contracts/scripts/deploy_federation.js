...
async function main() {
  ...
  const federation = await Federation.deploy({ gasPrice: 0 });
  await federation.waitForDeployment();

  const addr = await federation.getAddress();
  console.log("Federation deployed at:", addr);

  // Save address with <networkName>-<contractName>.json
  const networkName = hre.network.name;
  const outDir = path.join(__dirname, "..", "deployments");
  fs.mkdirSync(outDir, { recursive: true });
  const outfile = path.join(outDir, `${networkName}-Federation.json`);
  fs.writeFileSync(
    outfile,
    JSON.stringify(
      {
